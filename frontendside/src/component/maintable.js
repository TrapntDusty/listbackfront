...
import { getTasks, updateTask,softdelete } from '../Service/Api';

const MainTable = ({ sortingOption,searchTerm  }) => {
  ...
  const handleComplete = async (taskId) => {
    ...
    if (confirmed) {
      try {
        const updatedTaskData = { checkmark: true };
        ...
        // Update the local state to reflect the changes
        const updatedTasks = tasks.map((task) =>
          task.id === taskId ? { ...task, checkmark: true } : task
        );
        setTasks(updatedTasks);
      } catch (error) {
          ...
          <ul>
            {filteredTodos.map((tasks) => (
              ...
              <div className="border border-blue-300 p-4 rounded relative flex justify-between items-center">
                ...
                <button onClick={() => handleComplete(tasks.id)} className="cursor-pointer">
                    <svg 
                      xmlns="http://www.w3.org/2000/svg" 
                      fill="none" 
                      viewBox="0 0 22 22" 
                      stroke="currentColor" 
                      className="h-5 w-5 mb-2 mr-1">
